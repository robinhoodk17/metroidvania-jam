...
@export var left_right_action : GUIDEAction
@export var up_down_action : GUIDEAction
...
func run_state_machine(delta: float) -> void:
	...
	if current_action_state == action_state.HOOKED:
		...
		run_animation.travel("idle")
		return
	var run_direction = left_right_action.value_axis_1d
...
func action_state_machine(_delta: float) -> void:
	var _horizontal_direction : float = left_right_action.value_axis_1d
	var _vertical_direction : float = up_down_action.value_axis_1d
...
func manage_action_inputs():
	if up_down_action.is_triggered():
...
func handle_gravity(delta: float) -> void:
	...
	if jumping_time < jump_floatiness:
		jumping_time += delta
		if up_down_action.value_axis_1d > 0.5:
			velocity.y = jump_velocity
	if is_on_floor():
		if coyote_timer.is_stopped():
			pass
